 ...
 * @author Ciccio
 */
public class PriceIncreaseStrategy implements PriceStrategyInterface {

    public PriceIncreaseStrategy() {
    }

    @Override
    public float calcolaImporto(Prenotazione p) {
        if(p!=null && p.getCampo().getPrezzoSingolaPartita()!=0){
            return (float) ((float)  p.getCampo().getPrezzoSingolaPartita() + p.getCampo().getPrezzoSingolaPartita()*0.10);
        }
        return 0;
    }
